						...
						/*ase_frag_input*/ ) : SV_Target
			{
				...
				surfaceData.clearCoatSmoothness = 1;
					...
					if (shadowIntensity != 1.0)
					{
						...
						float shadowAtten = MainLightShadow(shadowCoord, inputData.positionWS, inputData.shadowMask, inputData.normalWS);

						float targetShadow = (shadowIntensity > 1.0) ? 0.0 : 1.0;
						float blendFactor = abs(shadowIntensity - 1.0);
						float modifiedShadow = lerp(shadowAtten, targetShadow, saturate(blendFactor));
						color.rgb *= modifiedShadow;
					}
				#endif
